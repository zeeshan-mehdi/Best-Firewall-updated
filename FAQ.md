...
<a name="FAQ0"></a>
**(0) How do I use Best Firewall?**

* Enable the Best Firewall firewall using the switch in Best Firewall's action bar
* Allow (greenish\*) or deny (reddish\*) Wi-Fi or mobile internet access using the icons next to an application name in Best Firewall's applications list

You can use *Settings > Defaults* to change from block/blacklist mode (disable *Block Wi-Fi* and *Block mobile*, and then block unwanted applications in Best Firewall's applications list) to allow/whitelist mode (enable *Block Wi-Fi* and *Block mobile*, and then allow desired applications in Best Firewall's applications list).
...
<a name="FAQ1"></a>
**(1) Can Best Firewall completely protect my privacy?**

No - nothing can completely protect your privacy.
Best Firewall will do its best, but it is limited by the fact it must use the VPN service.
...
Android N and later allows Best Firewall to be an [Always-On VPN](https://developer.android.com/preview/features/afw.html#always-on-vpn).
...
<a name="FAQ2"></a>
**(2)  Can I use another VPN application while using Best Firewall**

If the VPN application is using the [VPN service](http://developer.android.com/reference/android/net/VpnService.html),
then no, because Best Firewall needs to use this service. Android allows only one application at a time to use this service.

Best Firewall is a firewall application, so there is no intention to add VPN support.
However, Best Firewall supports a [SOCKS5 proxy](https://en.wikipedia.org/wiki/SOCKS) to chain VPN applications.

<a name="FAQ3"></a>
**(3) Can I use Best Firewall on any Android version?**
...
<a name="FAQ4"></a>
**(4) Will Best Firewall use extra battery power?**

By default Best Firewall will hardly use any battery power.
All settings resulting in extra battery usage, like IP filtering and logging, have a warning.
If Best Firewall uses a lot of battery power, please double check your settings.
...
Note that Android often incorrectly contribute battery usage of other apps to Best Firewall,
because the network traffic of other apps is flowing through Best Firewall.
This means that it might look like Best Firewall is using a lot of battery power,
but that in fact the total battery usage of all apps is still the same.

<a name="FAQ6"></a>
**(6) Will Best Firewall send my internet traffic to an external (VPN) server?**
...
The [Android VPN service](http://developer.android.com/reference/android/net/VpnService.html) is being used to locally route all internet traffic to Best Firewall so no root is required to build this firewall application.
Best Firewall, unlike all other no-root firewalls applications, is 100% open source, so when you are in doubt you can check [the source code](https://github.com/M66B/NetGuard/) yourself.
...
By showing all applications, Best Firewall allows you to control internet access even *before* such an update occurs.

<a name="FAQ8"></a>
**(8) What do I need to enable for the Google Play™ store app to work?**

You need 3 packages (applications) enabled (use search in Best Firewall to find them quickly):
...
Note that Best Firewall does *not* require any Google service to be installed.
...
No, because if Tasker is allowed to disable Best Firewall, any application can disable Best Firewall.
Allowing a security application to be disabled by other applications is not a good idea.

<a name="FAQ13"></a>
**(13) How can I remove the ongoing Best Firewall entry in the notification screen?**

* Long click the Best Firewall notification
* Tap the 'i' icon
...
this operation will also disable any information or warning notifications from Best Firewall,
...
F-Droid builds are not supported because I have no control over if and when the F-Droid version of Best Firewall will be updated,
...
<a name="FAQ17"></a>
**(17) Why is Best Firewall using so much memory?**

It isn't.  Best Firewall doesn't allocate any memory, except a little for displaying the user interface elements and for buffering traffic.
...
and is incorrectly attributed to Best Firewall instead to the Google Play™ store app.

<a name="FAQ18"></a>
**(18) Why can't I find Best Firewall in the Google Play™ store app?**

Best Firewall requires at least Android 5.1, so it is not available in the Google Play™ store app on devices running prior Android versions.
...
You can block system applications and components, like Google Play services, by enabling the advanced Best Firewall option *Manage system apps*.
...
To be sure, you can disable battery optimizations for Best Firewall manually like this:

```
Android settings > Battery > three dot menu > Battery optimizations > Dropdown > All apps > Best Firewall > Don't optimize > Done
```

The procedure to accomplish this can vary between devices.

Disabling doze mode for Best Firewall cannot be done from within Best Firewall
because, according to Google, Best Firewall is [not an application type allowed to do this](http://developer.android.com/training/monitoring-device-state/doze-standby.html#whitelisting-cases).

<a name="FAQ22"></a>
**(22) Can I tether (use the Android hotspot) / use Wi-Fi calling while using Best Firewall?**

Yes, but you'll need to enable subnet routing and tethering in the Best Firewall network settings.
...
which is shown by Android and not by Best Firewall, unfortunately, cannot be removed.
...
because you can switch from block (blacklist) to allow (whitelist) mode using Best Firewall's settings.
...
To prevent a bad user experience, Best Firewall includes a predefined rule to default allow the Google connectivity services.
...
You can only purchase pro features when you have installed Best Firewall from the Google Play store.

<a name="FAQ30"></a>
**(30) Can I also run AFWall+ on the same device?**

Unless you are just testing Best Firewall, there is no current reason to use them both, since they cover the same function (firewall),
...
Also you need to keep per application access rules _always_ in sync between AFWall+ and Best Firewall,
...
Some pointers on how to set up AFWall+ to be used simultaneously with Best Firewall:
* if not using filtering in Best Firewall, applications _need_ direct internet access (Wi-Fi and/or mobile) in AFWall+
* if using filtering, Best Firewall will _need_ internet access (Wi-Fi and/or mobile) in AFWall+
* if using filtering, when you un/reinstall Best Firewall, remember to re-allow Best Firewall in AFWall+
* if using filtering, applications _need_ VPN internet access (check the box to show that option in AFWall+ settings)

This question was community contributed. There is no support on using Best Firewall and AFWall+ together.
...
<a name="FAQ32"></a>
**(32) Why is the battery/network usage of Best Firewall so high?**

This is because Android counts battery and network usage which is normally counted for other applications
against Best Firewall in IP filtering mode. The total battery usage is slightly higher when IP filtering mode is enabled.
...
Best Firewall "asks" Android to start the local VPN service,
but some Android versions contain a bug which prevents the VPN from starting (automatically).
Sometimes this is caused by updating Best Firewall.
Unfortunately this cannot be fixed by Best Firewall.
You can try to restart your device and/or revoke the VPN permissions from Best Firewall using the Android settings.
Sometimes it helps to uninstall and install Best Firewall again (be sure to export your settings first!).
...
Despite Best Firewall being *really* a lot of work, only some of the convenience and advanced features need to be purchased,
which means that Best Firewall is basically free to use
and that you don't need to pay anything to reduce your data usage, increase battery life, and increase your privacy.

Also note that most free applications will appear not to be sustainable in the end, whereas Best Firewall is properly maintained and supported,
...
<a name="FAQ38"></a>
**(38) Why did Best Firewall stop running?**

On most devices, Best Firewall will keep running in the background with its foreground service.
On some devices (in particular some Samsung models), where there are lots of applications competing for memory, Android may still stop Best Firewall as a last resort.
...
Unfortunately this cannot be fixed by Best Firewall, and can be considered a shortcoming of the device and/or as a bug in Android.
You can workaround this problem by enabling the watchdog in the Best Firewall advanced options to check every 10-15 minutes.
...
Access to domain names shown in the application access log (drill down in the Best Firewall application settings) will not be notified again,
...
<a name="FAQ45"></a>
**(45) Does Best Firewall handle incoming connections?**
...
If a purchased pro feature doesn't work [as described](https://www.shaikhutech.com/)
...
Developing Best Firewall was quite a challenge and [really a lot of work](https://www.openhub.net/p/netguard/estimated_cost), but fun to do.
...
Best Firewall blocks traffic based on the IP addresses an application is trying to connect to.
...
This could result in Best Firewall too early or too late purging a DNS record from its own cache,
resulting in not recognizing an IP address or recognizing a wrong IP address.
You can try to workaround this by changing the DNS TTL value setting of Best Firewall.
This value is used as a minimum DNS TTL value in an attempt to mimick the behavior of Android.

<a name="FAQ49"></a>
**(49) Does Best Firewall encrypt my internet traffic / hide my IP address?**

Best Firewall is a firewall application that filters internet traffic on your device (see also [this question](#FAQ6)),
so it is not meant to - and does not - encrypt your internet traffic or hide your IP address.

<a name="FAQ50"></a>
**(50) Will Best Firewall automatically start on boot?**

Yes, Best Firewall will automatically be started on boot if you powered off your device with Best Firewall enabled and Best Firewall is not installed on external storage.
...
<a name="FAQ51"></a>
**(51) Why does Best Firewall block all internet traffic?!**

Make sure you have put Best Firewall on the doze exception list (Android 6 Marshmallow or later)
and that Android allows Best Firewall to use the internet in the background (see also [this question](#FAQ21)).

Make sure you are not running Best Firewall in allow (whitelist) mode (check the Best Firewall default settings).
...
Mostly, you can workaround this bug by enabling filtering in Best Firewall's *Advanced options*.
...
Tor with Best Firewall is only supported in the [XDA Best Firewall forum](http://forum.xda-developers.com/showthread.php?t=3233012).
There is no personal support on Tor with Best Firewall, because I don't use Tor myself.
...
In Best Firewall's _Network options_ enable _Subnet routing_ and in _Advanced options_ toggle on
...
<a name="FAQ55"></a>
**(55) Why does Best Firewall connect to Amazon / ipinfo.io / 216.239.34.21?**

Best Firewall connects to Amazon / [ipinfo.io](https://ipinfo.io/) to show the names and organizations for IP addresses.
If you don't want this, just disable showing names and organizations using the three dot menu in the global log view.

<a name="FAQ56"></a>
**(56) Why does Best Firewall allow all internet traffic?!**

Best Firewall can block each and every application, even system applications and components.

Best Firewall, by default, allows all traffic to prevent hard to find problems.  You need to selectively block traffic yourself by tapping on the mobile or Wi-Fi icons.

Be aware that Best Firewall will allow traffic to an application when the screen is on and the condition *'when screen on'* is enabled.

<a name="FAQ57"></a>
**(57) Why does Best Firewall use so much data?**

Basically, Best Firewall doesn't use data itself.
However, many Android versions incorrectly account data of other applications flowing through Best Firewall to Best Firewall instead of to the applications.
The data usage of other applications will be zero with Best Firewall enabled in this case.

The total data usage of your device will be the same with and without Best Firewall.
...
* Open Best Firewall and navigate to the pro features screen; Best Firewall will check the purchases again
...
* An application like Best Firewall cannot select which Google account to use
...
<a name="FAQ61"></a>
**(61) Help, Best Firewall crashed!**

Best Firewall rarely crashes ("unexpectedly stopped"), but if it crashed (which is something different than being stopped by Android, see [this FAQ](#FAQ38)),
...
**Best Firewall is supported for phones and tablets only, so not for other device types like televisions or vehicles.**

**If you didn't find the answer to your question, you can ask your questions [in this forum](http://forum.xda-developers.com/showthread.php?t=3233012) or contact me by using [this contact form](https://contact.shaikhutech.com/)**.
